#!/usr/bin/env node
/**
 * Encoding pre-flight for dsh-console-utf8.
 *
 ...
 * "worked yesterday" into a host that refuses to start — which would be
 * bitterly ironic for a plugin about encoding. This sweeps every shipped text
 * file for a BOM, for replacement characters (a file decoded with the wrong
 * code page and re-saved), for tabs used as YAML indentation, and for JSON
 * that no longer parses.
 *
 * It also asserts the one thing this plugin's own subject matter makes easy to
 * get wrong: the generated shell hook and the JSON/YAML that mount it must not
 * carry a BOM either.
 *
...
for (const absolute of walk(root)) {
  ...
  if (rel === 'cordis.patch.yml') {
    ...
    if (!/^\s+- id: dsh-console-utf8$/m.test(text)) fail('cordis.patch.yml must insert the row id "dsh-console-utf8"')
    if (!/^\s+name: 'dsh-console-utf8'$/m.test(text)) {
      fail('cordis.patch.yml must insert the row name "dsh-console-utf8"')
    }
  }
}
